...
disease_model = tf.keras.models.load_model('models/Trained_model.keras')
disease_classes = [
...
def yield_prediction():
    ...
    if request.method == "POST":
        ...
        if top_5_crops:
            ...
            ax.set_title("Top 5 Recommended Crops by Estimated Production")
            ax.set_ylabel("Production (tons)")
            
           
            ax.bar_label(bars, padding=3, labels=[f'{x:,.0f}' for x in productions])
...
def disease():
    ...
    if request.method == "POST":
        ...
        if file:
            ...
            print(f"DEBUG: Predicted Index = {result_index}")
